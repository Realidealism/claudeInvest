...
- 2026-08-14 | 依 memory「`db/seed_themes.py` 是驗證式載入器、`--check` 只驗證」跑了 `python -m db.seed_themes --check`，把已清空的 themes 表灌回 33 題材/231 對照 | 以為 memory 描述的檔案狀態＝現在的檔案狀態（實際那個載入器從沒被寫出來，檔案仍是舊版硬編碼 seeder、沒有 argparse，未知旗標被靜默忽略） | 跑前先 `grep -n "argparse\|sys.argv\|--check" <script>` 確認旗標真的被解析，並看 `__main__` 區塊會呼叫什麼 | 執行任何**可能寫入 DB/共用產出**的腳本前 → MUST 先讀該腳本的進入點與參數解析，**不可用 memory/文件的描述代替讀檔**；「加了 dry-run 旗標」不構成安全，未被解析的旗標＝直接執行本體
- 2026-08-26 | 派 agent 反查「PCB 產業鏈漏收了哪些台股」，三輪共回報 3 檔「漏網之魚」實為已下市公司（先豐 2020-10、新揚科 2021-12、光譜 2026-04）| 以為 agent 附了官網／財經站來源就代表該公司仍在市 | 財經網站與公司官網不標示下市狀態，agent 無從分辨，靠 tw.stocks 的 delisted_date 才全數擋下 | 採信 agent 回報的任何**個股**（新增題材成員、族群清單、同業比較）前 → MUST 先用 tw.stocks 查 is_active／delisted_date／security_type，不可只看 agent 附的來源 URL；★「仍在市」也不等於「仍在營運」——8291 尚茂停產兩年、終止全員勞動契約、月營收僅數十萬元仍掛牌，收錄成分股時要另查營運現況
