...
**根因**：對模型來說「自己直接讀」比「派工再等回報」感覺比較快，於是跳過委派。這個直覺是錯的：省下的等待時間 < 主 context 被灌爆後後半段任務失智的代價。
...
**修法**：
- 動手改訊號/防守/出場邏輯前，先掃一遍 MEMORY.md 的失敗封存條目；提案命中 → 直接停，回報使用者「此方向已封存於 `<memory 檔名>`」，除非使用者明示重開。
